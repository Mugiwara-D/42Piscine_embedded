...
int main()
{
    ...
    while (1)
    {
        ...
        _delay_ms(1000);   // Delay to make the toggle visible
        PORTD = (PORTD & ~(1 << 3)) | (1 << 5) | (1 << 6);  // Toggle LED D5 yellow state
        _delay_ms(1000);   // Delay to make the toggle visible
        PORTD = (PORTD & ~(1 << 5)) | (1 << 3) | (1 << 6);  // Toggle LED D5 cyan state
        _delay_ms(1000);   // Delay to make the toggle visible
        PORTD = (PORTD & ~(1 << 6)) | (1 << 5) | (1 << 3);  // Toggle LED D5 magenta state
        _delay_ms(1000);   // Delay to make the toggle visible
        PORTD = (1 << 6) | (1 << 5) | (1 << 3);  // Toggle LED D5 white state
        _delay_ms(1000);   // Delay to make the toggle visible
    }
}