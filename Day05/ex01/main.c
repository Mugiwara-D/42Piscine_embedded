...
int main(void) 
{
    ...
    while (1) 
    {
        ...
        uint16_t pot_value = ADCH; // Read ADC_pot value
        ADMUX = (ADMUX & 0b11110000) | 0b00000001; // Switch to ADC1 (PC1)
        ADCSRA |= (1 << ADSC); // Start conversion
        while (ADCSRA & (1 << ADSC)); // Wait for conversion to complete
        uint16_t ldr_value = ADCH; // Read ADC_ldr value
        ADMUX = (ADMUX & 0b11110000) | 0b00000010; // Switch to ADC2 (PC2)
        ADCSRA |= (1 << ADSC); // Start conversion
        while (ADCSRA & (1 << ADSC)); // Wait for conversion to complete
        uint16_t ntc_value = ADCH; // Read ADC_ntc value
        ADMUX = (ADMUX & 0b11110000) | 0b00000000; // Switch back to ADC0 (PC0)
